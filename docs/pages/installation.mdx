...
2. Additional Dependencies for Building from Source
   - **jq** - Command-line JSON processor (required for building EVM fixtures)
   - **solc** - Solidity compiler version 0.8.21+ (required for compiling contract fixtures)
   - **anvil** - Foundry's Ethereum development node (required for EVM-related features)
